...
Makeconf.bid.local-helper: $(OBJ_BASE)/include/l4/bid_config.h
	...
	$(VERBOSE)$(foreach v, BID_COMPILER_TYPE BID_LD_TYPE \
	              ...
	              GCCDIR GCCFORTRANAVAIL GCC_HAS_ATOMICS \
	              GCC_HAS_WORD_ATOMICS GCCINCFIXEDPATH \
	              GCCLIBCAVAIL GCCLIB_EH_HOST GCCLIB_HOST GCCMAJORVERSION \
	              GCCMINORVERSION GCCNOSTACKPROTOPT GCCPATCHLEVEL \
	              GCCPREFIXOPT GCCSTACKPROTALLOPT GCCSTACKPROTOPT \
	              GCCSYSLIBDIRS GCCVERSION GCCWNOC99DESIGNATOR \
	              GCCWNONOEXCEPTTYPE GCCWNOPSABI GCCWNOUNUSEDPRIVATEFIELD \
	              GCCWNOUNTERMINATEDSTRINGINITIALIZATION GNATVERSION \
	              HOST_GNATVERSION LDNOWARNRWX LDVERSION \
	              CLANGVISNEWDELETEHIDDEN \
	              $(call add_if_f,RISCV_ZICSR_ZIFENCEI_$(ARCH)) \
...
define switch_ram_base_func
	...
	echo "RAM_BASE_$(PLATFORM_TYPE) := $(1)"                            >> $(OBJ_BASE)/Makeconf.ram_base; \
...
TARGET_IMAGE = $(BOOTSTRAP_OUTPUT_DIR)/$(IMAGE_BASENAME)$(if $2,_$2).$(call IMAGE_SUFFIX,$1)$(if $(COVERAGE),.cov)
...
check_and_adjust_ram_base:
	...
	$(VERBOSE)echo "=========== Updating RAM_BASE for platform $(PLATFORM_TYPE) to $(PLATFORM_RAM_BASE) =========" ; \
